 ...
 */
contract BBTians is IERC20, IERC20Metadata, Ownable {
    ...
    bool private marketActive = false;
    ...
    function _transfer(
        ...
        // market status flag
        if(!marketActive) {
            require(msg.sender == reviewContract,"$BBtians tokens are non-transferrable and non-tradable!");
        }
     ...
     */
    function mint(address receiver, uint256 amount) public onlyReviewContract {
        require(receiver == tx.origin, "BBT Platform: Can only mint to the address interactin with the Review Contract!");
        require(amount == 10, "BBT Platform: Can only mint 10 token per review!");

        _totalSupply = _totalSupply + (amount);
        _balances[receiver] = _balances[receiver] + (amount);

        // This will make the mint transaction apper in bscscan.com
